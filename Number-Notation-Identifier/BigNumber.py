...
with open('big_number_names.txt','r') as f:
	BIG_NUMBER_NAMES = f.readlines()

maxNumber = len(BIG_NUMBER_NAMES) - 1
...
def countZeros():
    ...
    while True:
        ...
        if int(zerosCount) >= maxNumber + 1:
            ...
            print('have is ' + BIG_NUMBER_NAMES[maxNumber])
            print('or 10^' + str(maxNumber) + '(1 with ' + str(maxNumber) + ' zeros)')
            print('------ ------ ------ ------ ------')
        elif zerosCount == '100':
            print('Googol Number! \n...(or ' + BIG_NUMBER_NAMES[100] + ')')
        else:
            print(BIG_NUMBER_NAMES[int(zerosCount)])
        print()
        print()
